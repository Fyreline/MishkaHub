...
3. **[Recommender v1](phases/PHASE-3-recommender.md)** — content-based + personalised recs, filtered to owned services + region GB. ✅ shipped: content-similarity ("more like this"), "Feeling Lucky", and a per-user taste model (Ridge + prototype blend, MMR-diversified) over a TMDB-discovered candidate pool (thousands of films, still growing). Everything respects "not seen, or not seen in ≥365 days." **Perf fix (2026-07-05):** the corpus fit + taste-model build had never been cached, and the module docstrings' original "fine at tens to low hundreds of films" assumption was silently invalidated once the corpus reached ~5,000 — measured live at ~6s per `/api/recommendations` call and ~1.5s per `/api/films/{id}/similar` call. Now cached (corpus fit shared + TTL'd across both endpoints, taste models TTL'd and invalidated on retrain) and the lazy availability-refresh loop fetches concurrently instead of one-TMDB-call-at-a-time; measured warm-cache result ~0.85s and ~0.2s respectively. Evaluation harness (§8) and the nightly retrain scheduler (§6) are the main pieces still deferred — the new in-process TTL cache is a stopgap for the latter, not a replacement.
4. **[Accounts + feedback](phases/PHASE-4-accounts-feedback.md)** — login (2 fixed users, JWT), ratings/likes, active learning. 🟡 accounts/login shipped (2026-07-05) — argon2id + JWT access/refresh with rotation and reuse-detection, real per-user gate on every endpoint, zero registration surface (passwords set once via `scripts/set_password.py`, run locally). The interim bearer-token guard is fully deleted, not superseded. Still ⬜ not built: the feedback-event model-effect wiring and active-learning prompt system (§5-§6 of the phase doc) — rating/liked/watched already work (Phase 2/3), just not yet feeding `feedback_events`/prompts.
...
6. **[Service optimisation](phases/PHASE-6-service-optimisation.md)** — subscribe/drop suggestions. ✅ shipped (2026-07-05) — a Settings page lets the household pick which streaming services they subscribe to (backed by `GET/PUT /api/settings/subscriptions`, `GET /api/providers`), which the whole availability/recommendation pipeline respects; a new "Services" tab (`GET /api/insights/services`, `pipeline.service_insights`) ranks unsubscribed services by how many good recommendations they'd uniquely unlock ("worth adding") and subscribed services by how few of the household's top recommendations depend on them alone ("worth dropping") — reusing the existing taste-score model rather than a new ranking system. Verified live against the real household data.
7. **[Local media → TV](phases/PHASE-7-local-media-tv.md)** — Jellyfin to the webOS TV, "Play on TV". 🟡 v1 shipped (2026-07-04) — an "Owned" tab lets the household configure media folders, scan for video files, and match them to TMDB films (Jellyfin library join when configured, filename-parse fallback, manual search backstop); owned films count as available in recommendations with zero streaming coverage. Still ⬜ not done: the household hasn't set up their own Jellyfin server yet (this environment can't reach one to test against), so "Play on TV" (`POST /api/media/play`, `JellyfinClient`) is real code but unverified against a live server, and ffprobe-based codec detection is skipped.
8. **[Streaming "coming soon"](phases/PHASE-8-coming-soon.md)** (stretch) — arrivals & upcoming dates. 🟡 v1 shipped (2026-07-04) as a much smaller subset — a "Coming soon" tab lists upcoming *theatrical* GB releases (TMDB `/movie/upcoming`), clearly labelled as cinema dates. The real target (streaming arrivals, personalised, watchlist alerts) still needs the weekly snapshot-diff job + new `coming_soon` table described in the phase doc's Tiers 1-3 — ⬜ not started.
